...
class FTANet(nn.Module):
    def __init__(self, device, freq_bin = 320, time_segment = 128):
        ...
        weight = torch.zeros((321, 128), dtype=torch.float32)
        weight[1:,:] += 777346 / 1070144
        weight[0,:] += 1070144 / 777346
        self.loss_fn = nn.BCEWithLogitsLoss(weight=weight.to(device))
    ...
    def forward(self, batch, requires_loss=True):
        ...
        x = self.sf_7([x_r, x_t, x_f]).squeeze(dim=1)
