...
def STFT(x, fr, fs, Hop, h):        
    ...
    N = int(fs/float(fr))
    window_size = len(h)
    f = fs*np.linspace(0, 0.5, np.round(N/2).astype(np.int32), endpoint=True)
...
def cfp_process(fpath, sr=None, hop=256, model_type='vocal'):
...
if __name__ == '__main__':
    ...
    parser.add_argument('-i', '--audio_dir', type=str)
    parser.add_argument('-o', '--cfp_dir', type=str)

    args = parser.parse_args()

    extract_cfp(
        audio_dir=args.audio_dir,
        cfp_dir=args.cfp_dir
    )